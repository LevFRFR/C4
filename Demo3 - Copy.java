...
public class Demo3 extends Application {
    ...
    public void start (Stage stage) {
        ...
        VBox vb = new VBox();
        Label label1 = new Label("Try Not to Die Simulator");
					...
					public void handle(long now) {
					 ...
					 if (whichDirection != Direction.NONE) {
						 ...
						 if (isItValid == 2) {
							 ...
							 if(gamemap.getPlayer().getY()== 0 ||gamemap.getPlayer().getY()== 1) {
								 if(gamemap.getPlayer().getX()== 8) {
									 gamemap.setPlayerLocation(10,10);
									 player1.relocate(10*46, 10*85);
								 }
								 if(gamemap.getPlayer().getX()== 9) {
									 gamemap.setPlayerLocation(9,9);
									 player1.relocate(9*46, 9*85);
								 }
								 if(gamemap.getPlayer().getX()== 10) {
									 gamemap.setPlayerLocation(8,10);
									 player1.relocate(8*46, 10*85);
								 }

							 }
							 if(gamemap.getPlayer().getY()== 10 ||gamemap.getPlayer().getY()== 9) {
								 if(gamemap.getPlayer().getX()== 8) {
									 gamemap.setPlayerLocation(10,0);
									 player1.relocate(10*46, 0*85);
								 }
								 if(gamemap.getPlayer().getX()== 9) {
									 gamemap.setPlayerLocation(9,1);
									 player1.relocate(9*46, 1*85);

							 }
								 if(gamemap.getPlayer().getX()== 10) {
									 gamemap.setPlayerLocation(8,0);
								 player1.relocate(8*46, 0*85);
							 }
						 }
   ...
   public void actualGame2(Stage gameStage2){
		setStartConditions();
		Group gamePane = new Group(player1, coins1, enemy1, moneycounter, spikes1);
		Scene gameScene = new Scene(gamePane, 46*20, 85*10);
